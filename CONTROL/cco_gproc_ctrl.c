...
{
   ctrl->pid = 0;
   if(ctrl->fileno != -1) { close(ctrl->fileno); }
}
#else
void EGPCtrlCleanup(EGPCtrl_p ctrl)
{
...
{
   /* fork() not available under Emscripten -- always return NULL (child role)
      so the caller runs the strategy directly in this process. */
   (void)cores; (void)cpu_limit;
   fprintf(GlobalOut, COMCHAR" Running %s in single-process mode (WASM)\n", name);
   return NULL;
}
#else
EGPCtrl_p EGPCtrlCreate(char *name, int cores, rlim_t cpu_limit)
